  ...
  if grep -Fq "${MARKER_BEGIN}" "${LW_CFG}" 2>/dev/null; then
    ...
    if [[ "${DRY_RUN}" -eq 1 ]]; then
    ...
    else
      {
        ...
        echo 'defaultPref("librewolf.webgl.prompt", false);'
        echo 'defaultPref("librewolf.webgl.prompt.hide", true);'
  ...
  case "${base}" in
    ...
    0029-darkstr-webgl-context-enable.patch)
      grep -Fq "Soft residual (0029)" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrModeXor.sys.mjs" \
        ...
        && grep -Fq "librewolf.webgl.prompt" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrModeXor.sys.mjs" \
      ...
      ;;
    0030-darkstr-persona-etld-seed-rotate.patch)
      grep -Fq "Soft residual (0030)" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrNativePersona.sys.mjs" \
        && grep -Fq "_rotationLocked" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrNativePersona.sys.mjs" \
        && grep -Fq "resolveSnapshotForUri" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrNativePersona.sys.mjs" \
        && grep -Fq "darkstr.persona.rotatePerSite" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrNativePersona.sys.mjs" \
        && grep -Fq "Soft residual (0030)" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrNativePersonaParent.sys.mjs"
      ;;
    0031-darkstr-audio-silence-farbling.patch)
      grep -Fq "Soft residual (0031)" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrDepthHooksChild.sys.mjs" \
        && grep -Fq "farbleFloat" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrDepthHooksChild.sys.mjs" \
        && grep -Fq "getByteTimeDomainData" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrDepthHooksChild.sys.mjs" \
        && grep -Fq "copyFromChannel" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrDepthHooksChild.sys.mjs"
      ;;
    0032-darkstr-webgl-ext-precision.patch)
      grep -Fq "Soft residual (0032)" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrDepthHooksChild.sys.mjs" \
        && grep -Fq "advertisedExtensions" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrDepthHooksChild.sys.mjs" \
        && grep -Fq "PRECISION_PROFILES" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrDepthHooksChild.sys.mjs" \
        && grep -Fq "BASELINE_WEBGL_EXTENSIONS" "${DARKSTR_GECKO_ROOT}/browser/components/DarkstrDepthHooksChild.sys.mjs"
      ;;
    0033-darkstr-nav-languages-cloneinto.patch)
...
shopt -s nullglob
for patchfile in "${STUBS}"/000*.patch "${STUBS}"/001*.patch "${STUBS}"/002*.patch "${STUBS}"/003*.patch "${ROOT}/patches"/000*.patch "${ROOT}/patches"/001*.patch "${ROOT}/patches"/002*.patch "${ROOT}/patches"/003*.patch; do
...
echo "Phase 3 WebGL context enable: patches/0029-darkstr-webgl-context-enable.patch (ModeXor webgl.disabled=false + force-enabled + gfx.blocklist.all=-1 + librewolf.webgl.prompt=false; nsIGfxInfo lastStatus). Rebuild: browser/components only; no XUL relink (see docs/PHASE-3-WEBGL-0029-MINI-APPLY.sh)."
echo "Phase 4 eTLD seed rotate: patches/0030-darkstr-persona-etld-seed-rotate.patch (sticky per-eTLD+1 seed; rotatePerSite; Proof lock via snapshot|rotatePerSite=false). Rebuild: browser/components only; no XUL relink (see docs/PHASE-4-ETLD-0030-MINI-APPLY.sh)."
